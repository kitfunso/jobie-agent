...
4. Click Apply on the posting yourself, choose Apply Manually and sign in. Then click Run to review. On each page it fills what the profile covers, then reads every field still empty (label, kind, options, required star) and asks a second agent to answer from your profile, your notes and your CV. Every tenant asks its own questions, so nothing is hard-coded: the agent returns one answer and one reason per field, null where the three sources say nothing, and Python enforces the dropdown options. It attaches the PDFs, presses Save and Continue, and lists what it could not answer. Profile, notes and CV are saved once, so each later application is Read, Run, Review.
5. It stops on the Review page, on a validation error, on a required field it could not answer, or on a page it does not recognise. Workday's Submit button shares an id with Save and Continue, so the extension reads the button text before every click and refuses anything that says Submit. You read the review page and submit it yourself.
...
- **Form answers.** A second `strands.Agent` with structured output only: `FormAnswers`, one answer and one reason per field, options enforced in Python before anything reaches the page.
- **Server.** FastAPI on 127.0.0.1:8765. Five endpoints: health, CV parse (pypdf), tailor, answer, and file download. PDFs rendered with reportlab.
...
- Run free-text question answers through the same checker as the letter.
